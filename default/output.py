...
# --- log_and_load_data replaced with reference version ---
...
# --- Insert logger.flush() after write operations ---
...
def log_and_write_unity_table(
    ...
    try:
        ...
        upload_count = df.count()
        log_data["df_count"] = upload_count
        df.write.format("delta").mode("overwrite").option("overwriteSchema", "true").saveAsTable(destination_path)
        info(
            f"Done uploading {log_data['job_name']}",
            data={
                "task": log_data["task"],
...
def log_and_write_csv_data(
    ...
    try:
        ...
        upload_count = df.count()
        log_data["df_count"] = upload_count
        df.write.format("csv").mode("overwrite").option("header", "true").save(destination_path)
        info(
            f"Done uploading {log_data['job_name']}",
            data={
                "task": log_data["task"],
...
# --- Insert logger.flush() in critical error blocks ---
def write_delta_data_with_error_handling(df: DataFrame, destination_path: str, log_data: dict) -> None:
    try:
        write_delta_data(df, destination_path, log_data)
    except Exception as e:
        error(
